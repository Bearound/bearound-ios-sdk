...
final class WifiCollector {
    ...
    func refresh() {
        if #available(iOS 14.0, *) {
            NEHotspotNetwork.fetchCurrent { [weak self] network in
                ...
                let observation = Self.observation(from: network?.bssid)
                self.lock.lock()
                self.cached = observation
                self.lock.unlock()
            }
        } else {
            let observation = Self.observation(from: Self.legacyBSSID())
            lock.lock()
            cached = observation
            lock.unlock()
        }
    ...
    private static func observation(from bssid: String?) -> WifiObservation? {
    ...
    private static func legacyBSSID() -> String? {
